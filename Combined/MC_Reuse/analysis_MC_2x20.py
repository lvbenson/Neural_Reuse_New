...
dir = "/N/u/lvbenson/Carbonate/Desktop/MC_2x20"
id = str(sys.argv[1])
reps = 100
...
def find_all_lesions(dir,ind):
    ...
    bi = np.load(dir+"/best_individualMC20_"+str(ind)+".npy")
...
gens = len(np.load(dir+"/average_historyMC20_0.npy"))
#print(gens)
gs=len(np.load(dir+"/best_individualMC20_0.npy"))
af = np.zeros((reps,gens))
bf = np.zeros((reps,gens))
bi = np.zeros((reps,gs))

af = np.load(dir+"/average_historyMC20_"+str(id)+".npy")
bf = np.load(dir+"/best_historyMC20_"+str(id)+".npy")
bi = np.load(dir+"/best_individualMC20_"+str(id)+".npy")
...
if bf[-1] > 0.0:
    ...
    np.save(dir+"/perfmap_MC_"+str(id)+".npy",m1)

    np.save(dir+"/state_MC_"+str(id)+".npy",ns1)
