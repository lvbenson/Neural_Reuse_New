...
def compare_analyses(Threshold):
    ...
    for i, file in enumerate(files):
        ...
        if np.min(fits) > 0.8:
            ...
            l_ip = np.load("./Combined/4T_2x5/Data/lesions_IP_" + str(ind) + ".npy")
            l_cp = np.load("./Combined/4T_2x5/Data/lesions_CP_" + str(ind) + ".npy")
            l_lw = np.load("./Combined/4T_2x5/Data/lesions_LW_" + str(ind) + ".npy")
            l_mc = np.load("./Combined/4T_2x5/Data/lesions_MC_" + str(ind) + ".npy")
            lesion_data = np.concatenate([l_ip, l_cp, l_lw, l_mc])
            lesion_bin_categs, lesion_categs = categorize(l_ip, l_cp, l_lw, l_mc, Threshold)

            mi_ip = 1 - np.load("./Combined/4T_2x5/Data/NormMI_IP_" + str(ind) + ".npy")
            mi_cp = 1 - np.load("./Combined/4T_2x5/Data/NormMI_CP_" + str(ind) + ".npy")
            mi_lw = 1 - np.load("./Combined/4T_2x5/Data/NormMI_LW_" + str(ind) + ".npy")
            mi_mc = 1 - np.load("./Combined/4T_2x5/Data/NormMI_mc_" + str(ind) + ".npy")
            mi_data = np.concatenate([mi_ip, mi_cp, mi_lw, mi_mc])
            mi_bin_categs, mi_categs = categorize(mi_ip, mi_cp, mi_lw, mi_mc, Threshold)

            v_ip = 1 - np.load("./Combined/4T_2x5/Data/NormVar_IP_" + str(ind) + ".npy")
            v_cp = 1 - np.load("./Combined/4T_2x5/Data/NormVar_CP_" + str(ind) + ".npy")
            v_lw = 1 - np.load("./Combined/4T_2x5/Data/NormVar_LW_" + str(ind) + ".npy")
            v_mc = 1 - np.load("./Combined/4T_2x5/Data/NormVar_MC_" + str(ind) + ".npy")
            ...
            plt.scatter(
                ...
                c="k" if ind == "0" else "C0",
                s=30,
                alpha=1 if ind == "0" else 0.8,
            )
    ...
    plt.tight_layout()
    plt.savefig("./Combined/4T_2x5/Figures/compare_reuse.pdf")
