...
#dir = '/Users/lvbenson/Research_Projects/Neural_Reuse_New/Combined/Cog_Models_Project/Data/Model2'
dir = str(sys.argv[1])
...
def analysis(genotype):
    ...
    return fitness,fit_CP,fit_MC,nn_state_cp,nn_state_mc
...
def find_all_var(dir,ind):
    ...
    nH = 10
    v = np.zeros((4,10))
    nn = np.load(dir+"/state_CP_{}.npy".format(ind))
    #gens = len(np.load(dir+"/average_history_0.npy"))
    v[0] = np.var(nn[:,nI:nI+nH],axis=0)
    nn = np.load(dir+"/state_MC_{}.npy".format(ind))
    ...
    np.save(dir+"/NormVar_CP_{}.npy".format(ind), norm_var[1])
    #np.save("./{}/NormVar_LW_{}.npy".format(dir,ind), norm_var[2])
    np.save(dir+"/NormVar_MC_{}.npy".format(ind), norm_var[3])
...
for i in range(start,finish):
    ...
    if bf[index][-1] > 0.0:
        ...
        count += 1
        f,m1,m2,ns1,ns2=analysis(bi[index])
        ...
        np.save(dir+"/perfmap_CP_"+str(i)+".npy",m1)
        #np.save(dir+"/perfmap_LW_"+str(i)+".npy",m3)
        np.save(dir+"/perfmap_MC_"+str(i)+".npy",m2)

        #np.save(dir+"/state_IP_"+str(i)+".npy",ns1)
        np.save(dir+"/state_CP_"+str(i)+".npy",ns1)
        #np.save(dir+"/state_LW_"+str(i)+".npy",ns3)
        np.save(dir+"/state_MC_"+str(i)+".npy",ns2)
