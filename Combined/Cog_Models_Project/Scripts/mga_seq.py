...
class Microbial():
    def __init__(self, fitnessFunction, popsize, genesize, recombProb, mutatProb, demeSize, generations, boundaries):
        ...
        self.pop = np.random.rand(popsize,genesize)*2 - 1 #size of population, so population * size of each genotype
        self.fitness = np.zeros(popsize) #fitness of entire each individual in the population 
    ...
    def run(self, task):
        ...
        for i in range(self.popsize):
            ...
            #runs evolve, calling the fitness function
            fits = self.fitnessFunction(self.pop[i])
            #self.fitness[i] = self.fitnessFunction(self.pop[i]) #calculates fitness for every individual. 
            self.fitness[i] = fits[task]
                ...
                for l in range(self.genesize):
                    ...
                    if self.boundaries:
                        ...
                        if self.pop[loser][l] < -1.0:
                            self.pop[loser][l] = -1.0
                # Save fitness
                fits_loser = self.fitnessFunction(self.pop[loser])
                #self.fitness[loser] = self.fitnessFunction(self.pop[loser])
                self.fitness[loser] = fits_loser[task]
