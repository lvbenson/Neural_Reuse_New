...
def all_mi_data():
    ...
    for i, file in enumerate(files):
        ...
        if np.min(fits) > 0.8:
            ...
            mi_data = np.load("./Combined/4T_2x10/Data/NormMI_IP_{}.npy".format(run_num))
            #print(mi_data)
            #print(mi_data.shape)
            plt.scatter(np.arange(1, 21), mi_data, c='blue', s=5, alpha=0.7)
            mi_data = np.load("./Combined/4T_2x10/Data/NormMI_CP_{}.npy".format(run_num))
            #print(mi_data)
            plt.scatter(np.arange(1, 21), mi_data, c='green', s=5, alpha=0.7)
            mi_data = np.load("./Combined/4T_2x10/Data/NormMI_LW_{}.npy".format(run_num))
            #print(mi_data)
            plt.scatter(np.arange(1, 21), mi_data, c='red', s=5, alpha=0.7)
            mi_data = np.load("./Combined/4T_2x10/Data/NormMI_MC_{}.npy".format(run_num))
            plt.scatter(np.arange(1, 21), mi_data, c='yellow', s=5, alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig("./Combined/4T_2x10/Figures/figure_6_MI.pdf")
    plt.show()
...
def plot_mi_analysis(run_num):
    ...
    # load and plot data
    mi_data = 1 - np.load("./Combined/4T_2x10/Data/NormMI_IP_{}.npy".format(run_num))
    plt.scatter(
        np.arange(1, 21) - 0.10, mi_data, s=50, alpha=0.7, c="xkcd:tomato", label="IP"
    )
    mi_data = 1 - np.load("./Combined/4T_2x10/Data/NormMI_CP_{}.npy".format(run_num))
    plt.scatter(np.arange(1, 21) - 0.05, mi_data, s=50, alpha=0.7, c="xkcd:azure", label="CP")
    mi_data = 1 - np.load("./Combined/4T_2x10/Data/NormMI_LW_{}.npy".format(run_num))
    plt.scatter(
        np.arange(1, 21),
        mi_data,
        s=50,
    ...
    mi_data = 1 - np.load("./Combined/4T_2x10/Data/NormMI_MC_{}.npy".format(run_num))
    plt.scatter(
        np.arange(1, 21) + 0.05,
        mi_data,
        s=50,
    ...
    for i, file in enumerate(files):
        ...
        if np.min(fits) > 0.8:
            ...
            ipp = 1 - np.load("./Combined/4T_2x10/Data/NormMI_IP_" + str(ind) + ".npy")
            cpp = 1 - np.load("./Combined/4T_2x10/Data/NormMI_CP_" + str(ind) + ".npy")
            lwp = 1 - np.load("./Combined/4T_2x10/Data/NormMI_LW_" + str(ind) + ".npy")
            mcp = 1 - np.load("./Combined/4T_2x10/Data/NormMI_MC_" + str(ind) + ".npy")
    ...
    plt.ylim([-0.5, 21.5])
    plt.tight_layout()
    plt.savefig("./Combined/4T_2x10/Figures/figure_6_MI_reuse.pdf")
    plt.show()
