...
def plot_mi_analysis(run_num):
    ...
    # load and plot data
    mi_data = 1 - np.load("../New/NormMI_IP_{}.npy".format(run_num))
    plt.scatter(
        np.arange(1, 11) - 0.05, mi_data, s=50, alpha=0.7, c="xkcd:tomato", label="IP"
    )
    mi_data = 1 - np.load("../New/NormMI_CP_{}.npy".format(run_num))
    plt.scatter(np.arange(1, 11), mi_data, s=50, alpha=0.7, c="xkcd:azure", label="CP")
    mi_data = 1 - np.load("../New/NormMI_LW_{}.npy".format(run_num))
    plt.scatter(
        np.arange(1, 11) + 0.05,
        mi_data,
        s=50,
    ...
    for i, file in enumerate(files):
        ...
        if np.min(fits) > 0.85:
            ...
            ipp = 1 - np.load("../New/NormMI_IP_" + str(ind) + ".npy")
            cpp = 1 - np.load("../New/NormMI_CP_" + str(ind) + ".npy")
            lwp = 1 - np.load("../New/NormMI_LW_" + str(ind) + ".npy")
